...
<table id="rounded-corner">
    ...
    <tbody>
        ...
        <tr>
            <td class="boder_right">MT: </td>
            <td colspan="3"><textarea onkeyup="demtin(this.value)" cols="55" rows="5"><%=StringEscapeUtils.escapeHtml(branLogdao.getMessage())%></textarea></td>
        </tr>
        <tr>
            <td class="boder_right">Số ký tự tin nhắn: </td>
            <td colspan="9"><span class="redBold" id="showtin"><%=StringEscapeUtils.escapeHtml(branLogdao.getMessage()).length()%></span></td>
        </tr>
    </tbody>
</table>
<%
