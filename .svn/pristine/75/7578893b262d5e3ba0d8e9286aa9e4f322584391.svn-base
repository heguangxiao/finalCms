...
//                one.setMessage(rs.getString("MESSAGE"));
    ...
    public static ArrayList<String> getTransIdProcess2(String accName) {
        ...
        try {
            ...
            rs = pstm.executeQuery();
            while (rs.next()) {
                result.add(rs.getString("TRAND_ID"));
            }
    ...
    public static ArrayList<String> getUsers() {
        ...
        PreparedStatement pstm = null;
        ResultSet rs = null;
        String sql = "SELECT DISTINCT USER_SENDER FROM MSG_RESEND_BY_ADMIN WHERE 1 = 1 ";
        try {
            conn = DBPool.getConnection();
            pstm = conn.prepareStatement(sql);
            int i = 1;
            rs = pstm.executeQuery();
            while (rs.next()) {
                result.add(rs.getString("USER_SENDER"));
            }
        } catch (SQLException e) {
            logger.error(Tool.getLogMessage(e));
        } finally {
