...
<html >
    <head>
        <% session.setAttribute("title", "Thông tin chiến dịch quảng cáo"); %>
    ...
    <body>
        <%  //---                 
            ...
            if (Tool.checkNull(endTime)) {
                endTime = DateProc.createDDMMYYYY();
            }
            ...
            ArrayList<Campaign> allCampaign = gDao.findByAll(currentPage, maxRow, campaignId, Campaign.TYPE.QC.val, "", startTime, endTime, "", "", status, userSender);
            
            int max = 20;
            int totalPage = 0;
            int totalRow = gDao.countByAll(campaignId, Campaign.TYPE.QC.val, "", startTime, endTime, "", "", status, userSender);
                            ...
                            <table id="rounded-corner" align="center">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="rounded-q4"><b>Tìm kiếm đơn hàng Quảng cáo</b></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                            ...
                            <table align="center" id="rounded-corner" summary="Msc Joint Stock Company" >
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td align="left" class="boder_right">                                            
                                            <a class="ItemPopup" 
                                               url-data="<%=request.getContextPath() + "/admin/statistic/mt/ViewMTQc.jsp?id=" + oneCP.getId()%>"
                                               href="#popup_content_view" id="open_popup_view" name="open_popup1">
                                                Xem MT
                                            </a>
                                        </td>
                                        ...
                                        <td class="boder_right">
                                            ...
                                            <a target="_blank" href="<%=request.getContextPath() + "/DownloadCamPaign?cpid=" + oneCP.getId()%>">
                                                <img width="32px" src="<%=request.getContextPath()%>/admin/resource/images/Downloads.png" alt="" title="Tải về" border="0">
                                            </a>
                                            <%}%>
                                        </td>
                                        <td class="boder_right">
        ...
        <script type="text/javascript">
            ...
            function fileUploadChoice(elm) {                
                ...
                var i = textinput.value.toString().lastIndexOf('.');
                
                var str = textinput.value.toString().slice(i+1, textinput.value.toString().length);
                
                if (str !== 'xlsx') {
                    document.getElementById('fileUpError').innerHTML = 'File không đúng định dạng. Chỉ up file .xlsx';
                    textinput.value = '';
                } else {
                    document.getElementById('fileUpError').innerHTML = '';
                }                
            }
            function uploadFile(cpid) {
                var file = $("#fileUpload_" + cpid).val();
                var textinput = document.getElementById("file_path_fake").value;
                if (file == null || file == "" || file == 'undefine' || textinput == '') {
