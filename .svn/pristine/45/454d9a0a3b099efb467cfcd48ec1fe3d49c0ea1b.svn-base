package gk.myname.vn.multipart.request;
 ...
 *
 * <pre>
 request = new HttpServletMultipartRequest2(request);

 for (Enumeration e = ((HttpServletMultipartRequest2) request)
 		.getFileParameterNames(); e.hasMoreElements();) {
 	String name = (String) e.nextElement();
 	MultipartFile multiFile = ((HttpServletMultipartRequest2) request)
 ...
 * @version 2.00b11
 */
public class HttpServletMultipartRequest2 extends HttpServletRequestWrapper {
...
//	public static final String DEFAULT_ENCODING = "ISO-8859-1";
    ...
    private final ParameterParser m_paramParser = new ParameterParser();
     ...
     * @throws IOException
     */
    public HttpServletMultipartRequest2(HttpServletRequest request)
            throws IOException {
     ...
     * @param maxContentLength - maximum number of bytes of content that can be
     * handled
     * @param maxContentMemoryThreshold - The maximum content length before
     * files will be automatically saved to disk. If the threshold is exceeded
     * all files will be written to disk.
     * @param onMaxLengthMode - The following options are available for this
     * option:
     * <ul>
     * <li>IGNORE_ON_MAX_LENGTH - If too much content is sent, this class will
     * not abort, but it means that no file content will be saved.</li>
     * <li>ABORT_ON_MAX_LENGTH - If max content breached, this setting will
     * cause the parse process to throw an exception.</li>
     * </ul>
     *
     * @param charEncoding - String encoding character set
     *
     * @param progressListener - observer pattern listener for progress events.
     * Events will fire every 4096 bytes or whenever a new parameter (file or
     ...
     * never happen) OR we have read past the Content-Length specified. (Should
     * also not happen). Otherwise return the number of characters read. You can
     * test whether the number returned is less than bytesToBeRead.length, which
     * indicates that we have read the last line of a file or parameter or a
     * border line, or some other formatting stuff.
     */
     ...
     *
     * @param endOfArray This is because in many cases the byteLine will have
     * garbage data at the end, so we act as though the actual end of the array
     * is this parameter. If you want to process the complete byteLine, specify
     * byteLine.length as the endOfArray parameter.
     */
...
//
//    private final ProgressListener m_progressListener;
