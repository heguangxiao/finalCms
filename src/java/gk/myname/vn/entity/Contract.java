...
public class Contract {
    ...
    public int getDausocodinh() {
        return dausocodinh;
    }

    public void setDausocodinh(int dausocodinh) {
        this.dausocodinh = dausocodinh;
    }
    ...
    public ArrayList<Contract> findList(int max, int page, String contract_no, String stRequest, String endRequest, int subID, int partnerID) {
        ArrayList<Contract> list = new ArrayList();
        Connection conn = null;
        PreparedStatement pstm = null;
        ResultSet rs = null;
        try {
            String sql = "select S.NAME AS NAME_SUB,P.NAME AS NAME_PARTNER,C.SUB_ID,C.PARTNER_ID,"
                    + "C.CONTRACT_NO,C.START_TIME,C.EXPIRE_TIME,C.AUTO_RENEW,C.OPTION_INFO,"
                    + "C.FILE_PATH,C.BRANDNAME,C.LONGCODE,C.DAUSOCODINH "
                    + "from contract C LEFT JOIN subsidiary S ON S.ID = C.SUB_ID "
                    + "LEFT JOIN partner_manager P on P.ID = C.PARTNER_ID "
                    + "where 1 = 1";
            ...
            while (rs.next()) {
                ...
                contr.setBrandname(rs.getInt("BRANDNAME"));
                contr.setLongcode(rs.getInt("LONGCODE"));
                contr.setDausocodinh(rs.getInt("DAUSOCODINH"));
                list.add(contr);
            }
        } catch (Exception e) {
            logger.error(Tool.getLogMessage(e));
    ...
    public int count(String contract_no, String stRequest, String endRequest, int subID, int partnerID) {
        int result = 0;
        Connection conn = null;
        PreparedStatement pstm = null;
        ResultSet rs = null;
    ...
    public boolean add(Contract contr) {
        ...
        PreparedStatement pstm = null;
        try {
            String sql = " INSERT INTO contract(SUB_ID,PARTNER_ID,CONTRACT_NO,START_TIME,EXPIRE_TIME,AUTO_RENEW,OPTION_INFO,FILE_PATH,BRANDNAME,LONGCODE,DAUSOCODINH) "
                    + "                  VALUES(     ?,         ?,          ?,         ?,          ?,         ?,          ?,        ?,        ?,       ?,          ?) ";
            conn = DBPool.getConnection();
            pstm = conn.prepareStatement(sql);
            int i = 1;
            ...
            pstm.setInt(i++, contr.getBrandname());
            pstm.setInt(i++, contr.getLongcode());
            pstm.setInt(i++, contr.getDausocodinh());
    ...
    public static Contract getID(int subID, int partnerID, String contractNo) {
        Contract contr = null;
        Connection conn = null;
        PreparedStatement pstm = null;
        ResultSet rs = null;
    ...
    public boolean update(Contract contr) {
        ...
        try {
            String sql = "UPDATE contract "
                    + " SET START_TIME = ? , EXPIRE_TIME = ? , AUTO_RENEW = ? , OPTION_INFO = ? , BRANDNAME = ? , LONGCODE = ? , DAUSOCODINH = ? ";
            ...
            sql += " WHERE SUB_ID = ? and PARTNER_ID = ? and CONTRACT_NO = ? ";
    ...
    public static ArrayList<Contract> list_partner(int subid) {
        ...
        ResultSet rs = null;
        try {
            String sql = "select P.NAME AS NAME_PARTNER,C.PARTNER_ID "
                    + "from contract C LEFT JOIN subsidiary S ON S.ID = C.SUB_ID "
                    + "LEFT JOIN partner_manager P on P.ID = C.PARTNER_ID "
                    + "where 1 = 1 ";
            if (subid != 0) {
