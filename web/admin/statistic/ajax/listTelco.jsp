...
<%
    if (!Tool.checkNull(code)) {
        ArrayList<Telco_Nations> allLabel = Telco_Nations.getTelcosByCountryCode(code);
        for (Telco_Nations elem : allLabel) {
%>
            <option value="<%=elem.getTelco_code()%>">[<%=elem.getCountry_code()%>] <%=elem.getTelco_name()%></option>
<%
        }
    } else {
